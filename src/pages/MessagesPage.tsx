...
export const MessagesPage: React.FC = () => {
  ...
  return (
    <div className="space-y-4 font-sans text-xs pb-12 select-none relative">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden flex flex-col md:flex-row h-[620px]">
        {/* Left Side: Conversation List */}
        <div className="w-full md:w-[320px] border-r border-gray-200 flex flex-col bg-gray-50/50">
              ...
              filteredConversations.map((c: any) => (
                ...
                >
                  <div className="h-9 w-9 rounded-full bg-brand-700 text-white font-extrabold text-xs flex items-center justify-center shrink-0 border shadow-xs">
                    {c.avatar || c.userName.slice(0, 2).toUpperCase()}
                  </div>
            ...
            <>
              ...
              <div className="p-3 border-b border-gray-200 flex items-center justify-between bg-white shadow-xs">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-full bg-brand-700 text-white font-extrabold text-xs flex items-center justify-center">
                    {activeConv.avatar || activeConv.userName.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="font-extrabold text-gray-900 text-xs">{activeConv.userName}</h3>
                    <span className="text-[10px] text-gray-400 font-bold block">{activeConv.userRole} • Active Now</span>
                  </div>
                </div>
              </div>

              {/* Chat Messages Feed */}
              <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-gray-50/30 scrollbar-thin">
                {activeConv.messages && activeConv.messages.length > 0 ? (
                  activeConv.messages.map((m: any) => (
                    ...
                    >
                      ...
                      <div
                        className={`max-w-[75%] p-3 rounded-lg text-[11.5px] font-semibold leading-relaxed shadow-sm break-words whitespace-pre-wrap ${
                          m.isMine 
              ...
              <form onSubmit={handleSend} className="p-3 border-t border-gray-200 bg-white flex items-center gap-2">
                <input
                  ...
                  className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-brand-500 font-medium bg-gray-50 focus:bg-white text-gray-900"
                />
                <button
                  type="submit"
                  disabled={!inputText.trim() || isSubmitting}
                  className="px-4 py-2 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-lg text-xs flex items-center gap-1.5 shadow-sm transition-colors cursor-pointer"
                >
